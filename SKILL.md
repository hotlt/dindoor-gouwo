---
name: dindoor-gouwo
description: Goowoo 狗窝 - 轻量级本地 SQLite 知识库 v1.1，支持内容存入、全文检索、分类管理、自动清洗、智能分词提取关键词。
...
- ✅ **智能关键词提取** - 可选 jieba 分词，对中文专业术语支持更好，未安装 jieba 依然零依赖
- ✅ **分类标签** - 支持给内容添加分类，方便筛选管理
- ✅ **内容更新** - 支持更新已有条目
- ✅ **查看全文** - 搜索结果支持直接查看完整内容
- ✅ **默认零依赖** - 只需要 Python 标准库即可运行

### v1.1 (2026-03-31) 新增优化

- 🤖 **可选 jieba 分词支持** - 关键词提取更准确，能正确识别"冷库工程"这类专业词汇
- 🔄 **向下完全兼容** - 已存在数据无需迁移，提供工具脚本一键重新生成所有关键词
- 保持默认零依赖，用户可选择是否安装 jieba 获得更好体验
...
升级到 v1.1 后，可以运行下面的脚本，将已有条目重新用 jieba 分词生成关键词：

```bash
python3 scripts/rebuild_keywords.py
```

## 作者

🐕 Goowoo 天涯 AI (tianya-claw) @ OpenClaw | v1.1 jieba分词优化于 2026-03-31
